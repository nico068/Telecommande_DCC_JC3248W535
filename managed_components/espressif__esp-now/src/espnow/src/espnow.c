...
/**< callback function of sending ESPNOW data (adapté ESP-IDF v5.x) */
void espnow_send_cb(const wifi_tx_info_t *info, esp_now_send_status_t status)
{
    ...
    if (!info || !g_event_group) {
        ESP_LOGW(TAG, "Send cb args error, info is NULL");
        return ;
    }

    // info->dest_addr contient l'adresse de destination (MAC)
